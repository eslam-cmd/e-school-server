...
export default function ViewPracticalQuiz() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/practical-quiz/${id}`, {
        method: "DELETE",
      });
  ...
  const validateEdit = () => {
    ...
    if (!editData.quiz_grade.toString().trim())
      errs.quiz_grade = "The mark is required.";
    setEditErrors(errs);
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: "grey.100", minHeight: "100vh" }}>
      <Box sx={{ maxWidth: "1200px", mx: "auto" }}>
                ...
                {filteredQuiz.map((quiz) => {
                  ...
                  return (
                    <TableRow key={quiz.id} hover>
                      ...
                      <TableCell sx={{ textAlign: "right" }}>
                        {student?.section || "Unspecified"}
                      </TableCell>
                      <TableCell sx={{ textAlign: "right" }}>
                        {student?.specialization || "Unspecified"}
                      </TableCell>{" "}
                      {/* ← الجديد */}
                      <TableCell sx={{ textAlign: "right" }}>
                        {quiz.quiz_title}
        ...
        >
          <DialogTitle>
            Edit a note for the student: {editData.name}
          </DialogTitle>
