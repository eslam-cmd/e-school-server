...
export default function ViewExams() {
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`${API_URL}/api/sabject/${id}`, {
        method: "DELETE",
      });
  ...
  const validateEdit = () => {
    ...
    if (!editData.sabject_title.trim())
      errs.sabject_title = "The title is required.";
    ...
    if (!editData.sabject_grade.toString().trim())
      errs.sabject_grade = "The mark is required.";
    setEditErrors(errs);
  ...
  return (
    <Box sx={{ p: 3, backgroundColor: "grey.100", minHeight: "100vh" }}>
      <Box sx={{ maxWidth: "1200px", mx: "auto" }}>
        ...
        >
          <DialogTitle>
            Adjust a note for the student: {editData.name}
          </DialogTitle>
